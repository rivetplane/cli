...
- Codex: the managed app-server reads `thread/tokenUsage/updated`. Rivetplane also reads `account/usage/read`, `account/rateLimits/read`, and rate-limit notifications when the installed version provides them. This was checked against bindings generated by Codex CLI 0.149.1. Older versions can omit account methods or individual token classes. The account response overlaps thread counters, so Rivetplane uses it only for non-overlapping estimated cost and uses the thread notification as token authority. `estimatedUsageCreditsMicros` is marked estimated and uses `CREDITS`, not a billing currency.
