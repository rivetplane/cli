...
test("collects cumulative Codex token usage from plain rollout discovery", async () => {
  ...
  try {
    await discovery.poll(); assert.equal(samples.length, 1); assert.deepEqual(samples[0]?.tokens, { input: 100, output: 20, reasoning: 5, cache_read: 90, cache_write: 0, total: 120 }); assert.deepEqual(samples[0]?.context, { window_size: 258_400, used_tokens: 120 }); assert.deepEqual(samples[0]?.quota, [{ name: "codex:primary", used_percent: 26, resets_at: "2026-09-01T14:13:40.000Z" }]);
