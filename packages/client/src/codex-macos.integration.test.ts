...
test("macOS managed Codex app-server end to end", { skip: process.platform !== "darwin" || process.env.RIVETPLANE_CODEX_INTEGRATION !== "1", timeout: 240_000 }, async () => {
  ...
  try {
    ...
    const question = await eventually(() => json<{ pending: { id: string; type: string } | null }>(`http://127.0.0.1:${port}/v1/sessions/${threadId}/pending`), (value) => value.pending?.type === "question", "question request");
